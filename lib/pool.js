    ...
    */
        ...
        _this.jobManager.on('newBlock', function (blockTemplate) {
            ...
            */
            ...
            if (shareData.blockSolved != 1) {
                    ...
                    SubmitBlock(shareData.blockSolveData, function () {
                        CheckBlockAccepted(shareData.x11hash, function (isAccepted, tx) {
                            ...
                            GetBlockTemplate(true, function (error, result, foundNewBlock) {
                                if (foundNewBlock)
                                    emitLog('Block notification after block submission via RPC');
                            });
                ...
                // console.log('submitting to RPC to check x11hash %s', shareData.x11hash);
                SubmitBlock(shareData.blockSolveData, function () {
                    CheckBlockAccepted(shareData.x11hash, function (isAccepted, tx) {
                        isValidBlock = isAccepted;
                        shareData.txHash = tx;
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('started', function () {
            ...
            client.on('difficultyChanged', function (diff) {
                ...
                GetAudit(myFullHash, function (sRecipient, nABNPassed, x11hash2, rpcBlockHash, nRpcTime2) {

                    // console.log('\n*** fullhash %s, name %s, HasABNCount %d, x11 %s, RECIPIENT = %s, ABN PASSED %d ***\n', myFullHash, params.name, nHasABN, x11hash2, sRecipient, nABNPassed);
        ...
        blockPollingIntervalId = setInterval(function () {
            GetBlockTemplate(false, function (error, result, foundNewBlock) {
                if (foundNewBlock) {
                    ...
                    emitLog('Block notification::DiscoveredNewBlock via RPC polling');
                }
            });
        }, pollingInterval);
    }
    ...
    function GetBlockTemplate(needNewBlock, callback) {
        ...
        if (dPoolConfig == 0) {
            ...
            return GetABNBlockForStratum(options.address, needNewBlock, callback);
        }
    ...
    function GetABNBlockForStratum(poolAddress, needNewBlock, callback) {
        getMinerData('', function (result, fgood) {
            if (!fgood) {
                emitErrorLog('GetAbnBlockForStratum call failed for daemon instance with error ');
                callback('error');
            } else {
                var processedNewBlock = _this.jobManager.processGBT(result, needNewBlock);
                ...
                function (results) {
                    if (results.error) {
                        ...
                        try {
                            ...
                            callback(sRecipient, dABNPassed, x11hash2, rpcBlockHash, nTime);
                        } catch (e) {
                            callback('', 0, '', '00', 0);
                        }
                    }
